...
setwd('Y:/IRB_00096551/UPDB_Pedigrees_10_2021/TrimmedPedigree')
...
write_delim(families, "Z:/temp/full_high_risk_ped.tsv", delim = "\t")
```
...
with open('Z:/temp/full_high_risk_ped.tsv') as full_fam_ped:
...
# if we try to save large numeric types scientific notation is introduced, so we change them to integers
families <- families %>% mutate_if(is.numeric, as.integer)
write_delim(families, "Z:/temp/parentsAdded_full_562503.tsv", delim = "\t")
```
...
```{python add missing parents}
with open('Z:/temp/parentsAdded_full_562503.tsv') as full_fam_ped:
    full_fam_ped.readline()
    egoDict = {}
    momDict = {}
    dadDict = {}
...
preparedPed <- ped[ped$kindredID %in% c(1506, 16366586, 41296831, 25322898, 30361650), ]
preparedPed <- select(preparedPed, c(1:5))
colnames(preparedPed) <- c("family", "id", "father", "mother", "sex")
...
fullInfo <- preparedPed %>% filter(kindredID == x)
...
# 3 = green
# 4 = blue
#fullInfo$color <- ifelse(is.na(fullInfo$Patient_ID), 1,
...
listOfIds <- ifelse(is.na(fullInfo$Patient_ID), 
                  "", 
                  paste0(fullInfo$egoUPDBID, "\n", fullInfo$DIST_ID, "\n",
                         fullInfo$dmType, "\n", fullInfo$min_egfr, "\n", fullInfo$last_egfr, "\n", 
                         fullInfo$last_acr, "\n", fullInfo$Slope, "\n", fullInfo$esrd, "\n"))
...
plot.pedigree(pedigree, id = listOfIds, col = fullInfo$color, affected = fullInfo$rapid_decliner, cex = 0.5, symbolsize = 1.75)
